...
# Tier 2: sh.itjust.works

In addition to the [Tier 1: Universal Rules](1-universal-rules.md), sh.itjust.works has the following rules and policies.

::: info Proposed Rule Changes
To propose a change to these guidelines or start a discussion about how they are applied, you can post in one of the following communities: [!fedecan@lemmy.ca](https://sh.itjust.works/c/fedecan@lemmy.ca), [!agora@sh.itjust.works](https://sh.itjust.works/c/agora).
:::
...
You cannot create communities for adult content on `sh.itjust.works`. While you can engage with such content from remote instances, all content must be labelled as per [Tier 1 Rule 10](1-universal-rules.md#rule-10-sensitive-content-labelling) (Sensitive Content Labelling).
...
Automated accounts must comply with [Tier 1 Rule 11](1-universal-rules.md#rule-11-ai-generated-and-automated-content). The account must be labelled as a bot using the [account settings](https://sh.itjust.works/settings), and it should have contact information so that users can reach out to the bot's operator with concerns. As per [Rule 3](#rule-3-vote-and-engagement-manipulation), you cannot vote using automated means.
...
- Propose changes to our federation policy by posting in the [!fedecan@lemmy.ca](https://sh.itjust.works/c/fedecan@lemmy.ca) or [!agora@sh.itjust.works](https://sh.itjust.works/c/agora) communities.

- View a live list of federated and blocked instances: [sh.itjust.works/instances](https://sh.itjust.works/instances)

- View announcements about federation decisions: [!announcements@sh.itjust.works](https://sh.itjust.works/c/announcements)
...
If your account is banned from `sh.itjust.works` and you would like to appeal the ban, please contact us on [Matrix](https://matrix.to/#/#sh.itjust.works:matrix.org). For instance level appeals, your appeal will be reviewed by someone who was not involved in the original decision where possible.
